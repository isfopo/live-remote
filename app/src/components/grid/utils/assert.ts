import type { GridSize, ItemSize } from "../types";
...
export function assertGridOptions(options: GridOptions) {
  ...
  if (
    ...
    throw new Error(
      "If `cols` === `0`, the `itemSize.width` parameter must be specified"
    );
  }
  ...
  if (
    ...
    (typeof rows === "object" &&
      Object.values(rows).includes(0) &&
      itemSize?.height === undefined)
  ) {
    throw new Error(
      "If `rows` === `0`, the `itemSize.height` parameter must be specified"
    );
  }
